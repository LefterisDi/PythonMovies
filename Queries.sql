...
FROM role rl , role tmp_rl , role gvn_rl , 
	 movie_has_genre tmp_mvhgen , movie_has_genre gvn_mvhgen ,
     (SELECT rl.actor_id AS ActorID
     
      FROM role rl
      
      WHERE (SELECT COUNT(DISTINCT ntmp_mvhgen.genre_id)
...
WHERE 	  rl.actor_id = currActor.ActorID
	  AND tmp_rl.actor_id = rl.actor_id
	  AND tmp_rl.movie_id = tmp_mvhgen.movie_id
      AND gvn_rl.actor_id = 3226
      AND gvn_rl.movie_id = gvn_mvhgen.movie_id
...
GROUP BY rl2.actor_id HAVING COUNT(DISTINCT mvhg1.genre_id) = COUNT(DISTINCT mvhg2.genre_id)
